<template>
  <div class="filter">
    <div>
      <q-dialog v-model="FilterDialogState" position="bottom" persistent :class="DarkMode && 'pt-dark-mode'">
        <q-card class="filter-card pt-bg-primary pt-text-primary">
          <q-card-section>
            <q-btn flat icon="arrow_back" class="back-icon text-white q-mt-md q-ml-md fixed-top-left"
              ...
              <div class="q-mt-md">
                ...
                <div class="flex" style="gap: 10px">
                  <div v-for="type in options" :key="type.id">
                    <q-btn flat no-caps size="10px" class="btn-types not-selected" :class="{ selected: type.selected }"
                      @click="selected(type)">{{ type.label }}</q-btn>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="isSearchVisible" class="q-mt-xl">
              ...
              <q-input class="pt-border-accent-0 pt-input-field full-width q-px-md q-mr-md" dense borderless
                v-model="keyword" placeholder="Search"><template v-slot:append>
                  <q-icon @click="searchKeyword()" class="pt-text-accent-0 cursor-pointer"
                    :class="!keyword && 'disabled'" name="search" />
                </template></q-input>
            </div>
          ...
          <div v-if="calendar" class="calendar">
            ...
            <q-date class="full-width no-shadow pt-bg-primary" mask="MMM DD, YYYY" v-model="dateRange"
              @update:model-value="filter('date-range')" range color="red-6" />
          </div>
        </q-card>
      </q-dialog>
    </div>
